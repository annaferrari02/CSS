...
In order to systematically identify semantic themes characterizing user-chatbot interactions, we employed BERTopic, a neural topic modeling approach that leverages transformer-based embeddings to capture latent conversational structure \cite{grootendorst2022bertopic}. After standard preprocessing (stopword removal, lemmatization), we applied BERTopic's three-stage pipeline: (1) UMAP dimensionality reduction, which projects high-dimensional embeddings into lower dimensional space while preserving the semantic structure, (2) HDBSCAN density-based clustering, which identifies semantically coherent clusters, and (3) class-based TF-IDF for topic representation, constraining the model to 20 topics aiming to better interpretability. Topic labels were generated via Mistral-Large (2024) few-shot prompting (for each discovered topic, the model received the top-10 keywords and generated a descriptive label of max 6 words, see Fig. ----aggiungere fig del prompt----). 
From a sociological perspective, this operationalizes conversational frames, revealing whether chatbots follow users' thematic agendas or introduce divergent themes, addressing the issue of thematic mirroring\cite{9031475, chu2025illusionsintimacyemotionalattachment}. 
\item Semantic alignment

To quantify whether users and chatbots discuss semantically similar content in terms of conversational convergence and co-constructed meaning we embed every conversation using the E5-base \cite{Wang2022TextEB}, a state-of-the-art embedding model that maps natural language into a 768-dimensional semantic space where cosine similarity reflects meaning overlap. Unlike topic modeling, which identifies discrete themes, semantic embeddings capture continuous semantic relatedness, allowing us to measure fine-grained alignment even when different words express similar concepts (e.g., "happy" vs. "joyful"). To assess global semantic alignment, we computed centroid embeddings for USER- and Chatbot-generated text separately by averaging all turn-level vectors, then calculated cosine similarity between centroids, a metric ranging from 0 (orthogonal meaning) to 1 (identical semantic space). For visualization, we applied UMAP to project high-dimensional embeddings into 2D space, preserving local semantic structure while revealing distributional patterns. The resulting scatter plot shows whether USER and Chatbot expressions occupy overlapping regions (indicating shared semantic space) or form distinct clusters (suggesting divergent conversational content). This analysis complements topic modeling by addressing the issue of "user mirroring" at a continuous, rather than categorical, level: while topic modeling reveals what themes dominate, semantic alignment reveals how closely the parties' overall language use converges, operationalizing the concept of "socio-affective alignment", i.e. how an AI system
behaves within the social and psychological ecosystem co-created with itsuser, where preferences and perceptions
evolve through mutual influence \cite{socioaffective}. 
\end{enumerate}
