...
if __name__ == '__main__':
    ...
    DATA_PATH = "C:/Users/anna2/OneDrive/Desktop/CSS/human_ai_chatlogs.csv"
    df = pd.read_csv(DATA_PATH, usecols=["party","text"])

    # OPTION 1: With multiprocessing (should work now)
    #with Pool(cpu_count()) as pool:
    #    df["clean_text"] = pool.map(clean_text, df["text"].astype(str).tolist())
    
    # OPTION 2: Without multiprocessing (fallback, uncomment if above fails)
